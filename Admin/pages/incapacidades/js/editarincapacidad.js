		...
		AddClicks: function()
		{
			...
			$('.boton-finalizar').on("click", function(){
				...
				if(incapacidades.ValidarFinalizado())
				{
					...
					swal({title: "¿Está seguro que desea EDITAR la incapacidad?",
						text: "",
						type: "warning",
						...
						if (isConfirm) {
							...
							$.ajax({
								...
								type: 'POST',
								data: {
									bandera: "guardar-incapacidad-editada",
									json  : incapacidades.TomarDatos_Incapacidades(),
									tipo: $('#incapacidad-title').data('tipo'),
									fechacorte : $('#incapacidad-title').data('corte'), 
...
Cargar : function()
{
			...
			//aqui debo cargar todos los datos del pago
			$.ajax({
				url: 'pages/incapacidades/peticiones/peticiones.php',
				type: 'POST',
				data: {
					bandera: "editar-incapacidad",
					id: $('#incapacidad-title').data('id'),
					tipo: $('#incapacidad-title').data('tipo'),
					fechacorte: $('#incapacidad-title').data('corte'),
					...
					success: function (resp) {
						...
						if (resp.salida === true && resp.mensaje === true) {
						 ...
						 for (var i = 0; i < resp.datosincapacidad.length; i++) {
								...
								$('.incapacidad-number').val(resp.datosincapacidad[i].idincapacidad);
								$('#select-worker').val(resp.datosincapacidad[i].trabajador)
								$('#select-worker').change();
								$('#select-customer').val(resp.datosincapacidad[i].cliente)
								$('#select-customer').change();
								$('#select-city').val(resp.datosincapacidad[i].ciudad)
								$('#select-city').change();
								$('#select-eps').val(resp.datosincapacidad[i].eps)
								$('#select-eps').change();
								$('#select-type').val(resp.datosincapacidad[i].tipo)
								$('#select-type').change();
								$('#initial-date').val(resp.datosincapacidad[i].fechainicial);
								$('#final-date').val(resp.datosincapacidad[i].fechafinal);
								$('#cut-date').val(resp.datosincapacidad[i].fechacorte);
								$('#daysincapacidad').val(resp.datosincapacidad[i].dias);
								$('.incapacidad-full-value').val(resp.datosincapacidad[i].valor);

							}

						} else {
						 
					 	swal("Importante!", "Ha ocurrido un error, intenta nuevamente.", "info");
					}

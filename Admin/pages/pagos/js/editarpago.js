					...
					success: function (resp) {
						...
						if (resp.salida === true && resp.mensaje === true) {
							...
							for (var i = 0; i < resp.datospago.length; i++) {
								...
								$('.payment-full-value').val(resp.datospago[i].valor);
								$('.payment-date').val(resp.datospago[i].fechapago)
								$('.payment-eps').val(resp.datospago[i].eps);
								$('.payment-eps').change();
							...
							var value = $('.payment-full-value').val();
							$('.payment-value-2').text(value);
							$('.payment-total-value').text("");
							$('.payment-total-value').text(total);
							var difference = $('.payment-value-2').text() - $('.payment-total-value').text();
